...
source(here("R/core.R"))
reptime_path <- here("data/replicationTiming")

##### Human replication information from the hg38 liftover output from hg19 repliseq data
...
path = here("../../repliseq_data/hg38_repliseq_liftover/")
...
for(f in files){
  ...
  # load signal track
  signal <- rtracklayer::import(paste0(path, f, "_hg38.bigWig"))
...
saveRDS(rep_rle, file=paste0(reptime_path, "/replicationInfo_hg38.RDS"))

saveRDS(rep_gr, file=paste0(reptime_path, "/replicationInfo_gr_hg38.RDS"))
